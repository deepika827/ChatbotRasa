...
def get_pdf_text(filename="BankingLaw.pdf"):
    """
    Fetch the extracted text of the uploaded PDF from MongoDB.
    Handles both list (per page) and single string storage formats.
    """
    try:
        client = MongoClient("mongodb://localhost:27017/")
        db = client["mcp_database"]
        doc = db["pdf_texts"].find_one({"filename": filename})
        ...
        # If it's a list of pages, merge them into one string
